...
al., 2022; Rigby and Stasinopoulos, 2005{]} on the N = 27,434
participants from the primary sample with complete education and site
data. For HC and LV (unadjusted):
...
Calibration plots (Figure S2) assess whether the normative models are
well-specified: for each nominal centile (e.g., the 10th percentile
curve), the plot shows what proportion of observations actually fall
below that curve. A well-calibrated model produces points along the
diagonal (observed proportion = expected proportion). Our models closely
track the diagonal across all centiles, indicating that the fitted
distributions accurately capture the true variability in the data.
...
(\(\beta\) = -0.075), consistent with HVR capturing both hippocampal and
ventricular age-related changes, both of which accelerate at older ages.
Cognitive factors showed comparable quadratic effects across models
(\emph{g}: \(\beta\) \(\approx\) -0.078; Speed\textsubscript{s}:
\(\beta\) \(\approx\) -0.046).
