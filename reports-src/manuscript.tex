...
UK Biobank structural MRI data (N = 27,680; 56\% female; ages 45--82),
...
ICV-matched subsample (\(d = 0.19\)), confirming this effect is not a
...
and Pearson r = 0.99 (Fernandez-Lozano et al., 2025). Outputs included
bilateral hippocampal volumes (left, right, total) and temporal horn
volumes of the lateral ventricles (left, right, total). Intracranial
volume (ICV) was obtained from AssemblyNet (Coupé et al., 2020), a
large-ensemble convolutional neural network for whole-brain MRI
segmentation.
...
carriers show faster hippocampal loss than male carriers in aMCI (Park
et al., 2025), indicating that genetic risk can modulate typical
...
in aging-related patterns. Well-calibrated centile estimates (Figure S1)
have demonstrated clinical utility in detecting case-control differences
and predicting diagnostic transition from MCI to Alzheimer's disease
(Bethlehem et al., 2022; Dima et al., 2022). HVR showed slightly lower
test-retest reliability (ICC \textgreater{} 0.93; Table S3) than HC or
LV alone over a mean 2.7-year follow-up. Because this interval spans
real aging-related change, the lower ICC may partly reflect HVR's
sensitivity to concurrent hippocampal atrophy and ventricular expansion
rather than measurement noise alone --- a hypothesis that dedicated
longitudinal studies with shorter retest intervals could directly test.
...
associations in both sexes but with a female advantage (\(\beta\) =
0.048 vs.~0.027). HVR showed the opposite pattern, with males exhibiting
stronger associations (\(\beta\) = 0.045 vs.~0.036), though both sexes
...
correlate differently with outcomes, though HVR showed approximately
normal distributions here. All brain-cognition associations were small
(\(\beta\) = 0.02--0.04), explaining less than 1\% of variance in
cognitive performance. While statistically significant given the large
sample, the practical and clinical significance of these associations
...
and a donation from the Famille Louise \& Andre Charron.
...
et al. (2022). Subcortical volumes across the lifespan: {Data} from
18,605 healthy individuals aged 3--90 years. \emph{Human Brain Mapping},
\emph{43}(1), 452--469. \url{https://doi.org/10.1002/hbm.25320}
